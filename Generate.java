...
public class Generate {
	...
	public static void main(String[] args) throws Exception {
		...
		createBuildConfig(PROJECT_ROOT + "/buildconfig");
		createGradleProperties(PROJECT_ROOT);
	...
	private static void createBuildConfig(String path) throws Exception{
		Template template = getTemplate("dependencies.gradle.ftl");
		File file = new File(path+"/dependencies.gradle");
		if (!file.exists()){
			file.createNewFile();
		}
		FileWriter fileWriter = new FileWriter(file);
	...
	private static void createGradleProperties(String path) throws Exception{
		Template template = getTemplate("gradle.properties.ftl");
		File file = new File(path+"/gradle.properties");
		if (!file.exists()){
			file.createNewFile();
		}
		FileWriter fileWriter = new FileWriter(file);
	...
	public static void createApplication() throws Exception{
		Template template = getTemplate("MyApplication.kt.ftl");
		String [] packages = PACKGE_NAME.split("\\.");
		String applicationPath = PROJECT_ROOT+"/app/src/main/java";
		for(int i=0;i<packages.length;i++) {
			applicationPath = applicationPath+"/"+packages[i];
		}
		File applicationFile = new File(applicationPath+"/MyApplication.kt");
		if(!applicationFile.exists()) {
			applicationFile.createNewFile();
		}
		FileWriter fileWriter = new FileWriter(applicationFile);
		Map<Object, Object> map = new HashMap<>();
		map.put("packageName", PACKGE_NAME);
		template.process(map, fileWriter);
		fileWriter.close();
